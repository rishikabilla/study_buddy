...
function App() {
  ...
  // 2. Fetch Notes for the active course
  const fetchNotes = async (courseId) => {
    try {
      const res = await axios.get(`${API_BASE}/notes/${courseId}`);
      setNotes(res.data);
      // We don't automatically set activeNote to null here 
      // so the editor stays open after a Save action.
    } catch (err) {
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', fontFamily: 'sans-serif', overflow: 'hidden' }}>
      ...
      <div style={{ width: '280px', borderRight: '1px solid #ddd', padding: '15px', background: '#fff', overflowY: 'auto' }}>
        <button 
          onClick={addNote} 
          style={{ width: '100%', padding: '10px', marginBottom: '15px', cursor: 'pointer', background: '#edf2f7', border: '1px solid #cbd5e0', borderRadius: '4px', fontWeight: 'bold' }}
        >
          + New Note
        </button>
        ...
        {notes.map(n => (
          <div key={n.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid #eee' }}>
            <div 
              ...
              style={{ flex: 1, padding: '12px 10px', cursor: 'pointer', background: activeNote?.id === n.id ? '#edf2f7' : 'transparent' }}
            >
              {n.title || "Untitled Note"}
            </div>
            <button 
              onClick={() => deleteNote(n.id)} 
              style={{ background: 'none', border: 'none', color: '#e53e3e', cursor: 'pointer', padding: '10px', fontSize: '16px' }}
            >
              ✕
            </button>
          </div>
      ...
      <div style={{ flex: 1, padding: '20px', display: 'flex', flexDirection: 'column', background: '#f7fafc' }}>
        {activeNote ? (
          <>
            {/* Editor Header: Title Input + Close Button */}
